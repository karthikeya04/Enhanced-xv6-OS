...
int clone(void(*fn)(int*), int* arg, void* stack){
  ...
  *np->tf = *curproc->tf;
  ...
  // Set thread id and hread-group id.
  thread_id = np->pid;
  np->tgid = curproc->tgid;

  // Setting up the stack of the thread
  np->user_stack = stack;
  int *fake_return = stack + 4096 - 2*sizeof(int);
  int *argument = stack + 4096 - sizeof(int);
  *fake_return = 0xffffffff;
  *argument = *arg;


  // Setting up the trap-frame of the new thread.
  np->tf->eip = (int)fn;
  np->tf->esp = (int)fake_return;
  np->tf->ebp = np->tf->esp;
...
int join(void){
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->tgid != myproc()->pid || p->pid == myproc()->pid)
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
...
block_sem(void* chan)
{
  /* Your Code */
  acquire(&ptable.lock);
  release_lock((struct lock_t *) chan);
  struct proc *curproc = myproc();
  sleep(uva2ka(curproc->pgdir, (char *) chan), &ptable.lock);
  release(&ptable.lock);
  acquire_lock((struct lock_t *) chan);
}

void 
signal_sem(void* chan)
{
  /* Your Code */
  struct proc *curproc = myproc();
  wakeup(uva2ka(curproc->pgdir, (char *) chan));
}